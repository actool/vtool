...
public class ImportAutFile_Thread {
	/***
		...
		// reads the configuration parameters from the first line of the file
				...
				if (msg != "") {
					...
					//try {
						...
						r1.initStaticVariables();
						while (r1.getTransitions().size() != (totalLines)) {
							r1.initStaticVariables();
							list = new ArrayList<>();							
							iolts = new IOLTS();
							iolts.setInitialState(new State_(configs[0]));
							try {
								...
								for (int i = 0; i < (int) totalLines / lineForThread; i++) {
									r1 = new ReadFile();
									r1.init(ini, ini + lineForThread, path, "a" + i);
									ini += lineForThread;
									r1.start();
									list.add(r1);
								...
								if ((totalLines) % lineForThread != 0) {
									r1 = new ReadFile();
									r1.init(ini, ini + (totalLines % lineForThread), path, "a");
									ini += lineForThread;
									r1.start();
									list.add(r1);
									//System.out.println("Thread ......");
								...
								/*if (transitions.size() == totalLines) {
									System.out.println("transition: " + transitions.size());
								} else {
									System.err.println("transition: " + transitions.size());
								}*/
								...
								/*for (String l : alphabet) {
									if(l.charAt(0) == Constants.OUTPUT_TAG || l.charAt(0) == Constants.INPUT_TAG) {
										iolts.addToAlphabet(l);
									}
								}*/
								...
								iolts.getInitialState();
								
								// for (Transition_ t : transitions_) {
								// lts.addTransition(t);
								// }

								/*if (transitions.size() == totalLines) {
									System.out.println("transition_: " + transitions.size());
								} else {
									System.err.println("transition_: " + transitions.size());
								}*/
...
//								System.out.println("segundos:" + convert + "\n");
...
//		// long startTime = System.nanoTime();
//
//		String path = "C:\\Users\\camil\\Desktop\\c_model50states.aut";
//
//		// count line of file (number of transitions)
//		BufferedReader reader = new BufferedReader(new FileReader(path));
//		int totalLines = 0;
...
//		int lineForThread = totalLines;
//
//		if (lineForThread > 500) {// if have more than 500 transitions use thread
//			lineForThread = 500;
//		}
//
//		ReadFile r1 = new ReadFile();
...
//			// long totalTime = endTime - startTime;
//			// long convert = TimeUnit.SECONDS.convert(totalTime, TimeUnit.NANOSECONDS);
//			// // // convert = TimeUnit.MILLISECONDS.convert(totalTime,
